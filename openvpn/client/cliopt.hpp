...
#include <openvpn/transport/client/sslcli.hpp>
...
#error no SSL library defined
      ...
      creds_locked(false)
  {
    ...
    cc.frame = frame;
    cc.context = Frame::READ_OPENVPN_BIO_MEMQ_STREAM;
  ...
  std::string load_transport_config()
  {
    ...
    else
    {
      if (transport_protocol.is_ssl())
      {
        ...
        sslconf->frame = frame;
        sslconf->stats = cli_stats;
        sslconf->socket_protect = socket_protect;
        transport_factory = sslconf;
      }
      else if (transport_protocol.is_udp())
      {
        // UDP transport
