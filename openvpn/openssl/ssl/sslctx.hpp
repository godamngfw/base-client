	...
	private:
		SSL(const OpenSSLContext& ctx)
		{
			...
			try {
				...
				ct_in = mem_bio(ctx.config.frame, ctx.config.context);
				ct_out = mem_bio(ctx.config.frame, ctx.config.context);
		...
		static BIO* mem_bio(const Frame::Ptr& frame, const unsigned int context)
		{
			BIO *bio = BIO_new(bmq_stream::BIO_s_memq());
			if (!bio)
				throw OpenSSLException("OpenSSLContext::SSL: BIO_new failed on bmq_stream");
			bmq_stream::memq_from_bio(bio)->set_frame(frame, context);
			return bio;
		}
